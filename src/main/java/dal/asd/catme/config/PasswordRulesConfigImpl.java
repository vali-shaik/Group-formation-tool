...
public class PasswordRulesConfigImpl implements IPasswordRulesConfig {
	...
	public boolean checkRecentPasswords(User user) throws CatmeException {
		...
		try 
		{
			...
			resultSet=connection.createStatement().executeQuery(PasswordRulesUtil.PASSWORD_HISTORY_QUERY+"'"+user.getBannerId()+"' "+PasswordRulesUtil.PASSWORD_HISTORY_ORDER_QUERY);

			while(resultSet.next())
			{
				if(passwordEncoder.matches(user.getPassword(), resultSet.getString(PasswordRulesUtil.PASSWORD_FIELD)))
				{
					flag=false;
					return flag;
				}
